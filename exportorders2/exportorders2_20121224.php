	...
	function getContent()
	{
		...
		if (isset($_POST['submitFilter']))
		{
			...
			if (isset($_POST['Download'])) //download generated report
			{
				...
				if (isset($_POST['envoyer'])) //envoi du fichier StockAZ suivi des commandes
				{
						...
						if (isset($_POST['updateTracking']))
						{
							...
							while (($data = fgetcsv($handle,0,';')) !== FALSE) 
							{
								if ($i > 0) //sauter la ligne d entete
								{
									...
									$this->_html.='<tr>';
									$this->_html.='<td>'.$this_order->id.'</td>';
									$this->_html.='<td>'.$this_order->id_customer.'</td>';
									$this->_html.='</tr>';
									$this_order->shipping_number=(int)$data[13];
									$this_order->update();
