...
class ExportAccounts extends Module
{
	...
	function getContent()
	{
		...
		{
			/*analyze de la reception des infos passées dans l'URL*/
			$fields=array();
			$titles=array();
			...
			if (!$orders) 
				echo "Exportation des factures pour Ciel....";
			else
					...
					foreach ($list_id_orders as $my_id_order)
						{
							...
							if ( $order->valid == 1 ) {
								/*Partie Génération Vente uniquement: Journal VT*/
								...
								foreach ($arr_compte as $local_compte)
								{
									...
									case '707000':
										...
										break;
									case '708500':
										$mouvement.=$order->total_shipping.';'.'C;'.str_pad($order->invoice_number,6,"0", STR_PAD_LEFT).';'.'Ports et frais accessoires fact.';
										break;
									case '445714':
										$mouvement.=($order->total_paid-($order->total_products+$order->total_shipping)).';'.'C;'.str_pad($order->invoice_number,6,"0", STR_PAD_LEFT).';'.'Tva collectée 19.6 %';
										break;
