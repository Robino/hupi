							...
							if ( $order->valid == 1 ) {
								...
								foreach ($arr_compte as $local_compte)
								{
									...
									case '708500':
										...
										break;
									case '445714':
										/*$mouvement.=round((($order->total_products*0.200)+($order->total_shipping-($order->total_shipping/1.200))),2).';'.'C;'.str_pad($order->invoice_number,6,"0", STR_PAD_LEFT).';'.'Tva collectÈe 20.0 %';*/
                                        /*$mouvement.=round($order->total_shipping-($order->total_shipping/1.200),2)+round($order->total_products*0.200,2).';'.'C;'.str_pad($order->invoice_number,6,"0", STR_PAD_LEFT).';'.'Tva collectÈe 20.0 %';*/
                                        $mouvement.=$order->total_paid-($order->total_products+round($order->total_shipping/1.200,2)).';'.'C;'.str_pad($order->invoice_number,6,"0", STR_PAD_LEFT).';'.'Tva collectée 20.0 %';
										break;
